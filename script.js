...
const Nstr= strings.filter((elements) => {
...
const S=numbers.map(elements=> Math.pow(elements,2));
console.log(S);
const a=numbers.map(function(elements){
  return elements**2
});
console.log(a);
...
const indexOfFirstSeven = ranks.findIndex((rank) => rank === 7);

console.log(indexOfFirstSeven); 
...
//3:8
const num = [2, 4, 6, 8];
num.forEach(function(elements,index){
...
const M=marks.some((marks)=> marks<45);
 console.log(M)
...
console.log(`Does person have a bike? ${hasBike}`);
console.log(`Does person have a house? ${hasHouse}`);


const name = person.name;
const vehicle = person.vehicles.plane;
const output = `${name} travels by plane called  Air123`;
console.log(output)
console.log(person.name + "travels by plane called Air123");
